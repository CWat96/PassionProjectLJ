...
class PlatingMaterialResourceIT {
    ...
    @Mock
    private PlatingMaterialRepository platingMaterialRepositoryMock;
    ...
    void getAllPlatingMaterialsWithEagerRelationshipsIsEnabled() throws Exception {
        when(platingMaterialRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPlatingMaterialMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(platingMaterialRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllPlatingMaterialsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(platingMaterialRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restPlatingMaterialMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(platingMaterialRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
