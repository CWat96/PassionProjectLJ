...
class StoneGemResourceIT {
    ...
    @Mock
    private StoneGemRepository stoneGemRepositoryMock;
    ...
    void getAllStoneGemsWithEagerRelationshipsIsEnabled() throws Exception {
        when(stoneGemRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restStoneGemMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(stoneGemRepositoryMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllStoneGemsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(stoneGemRepositoryMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restStoneGemMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(stoneGemRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
